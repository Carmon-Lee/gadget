...
/**
 * @author guang.li
 * @version AbstractQueuedSynchronizer.java v 1.0 2020/10/13 10:14
 */
public abstract class MyAbstractQueuedSynchronizer
    ...
    /**
     * 将当前节点置为head节点，从而实现出队。
     * 注意AQS队列的伸缩方式，新线程从尾部入队，从头部出队
     *
     * @param node the node
     */
    ...
    /**
     * 进入park状态的线程，从unpark方法调用中恢复过来的时候，是不会抛出中断异常的
     * 因此需要调用 Thread.interrupted 这个方法获取线程的中断状态
     */
    ...
    /**
     * 已经加入等到队列，通过自旋获取锁机会
     * 假如当前线程是排队线程当中的第二个，那么可以考虑尝试获取锁
     * 否则直接判断前驱节点的状态，并根据状态值判断是否需要进行park
     */
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (; ; ) {
                ...
                // 首先检查获取失败之后是否应该park，为true则进入park状态，
                // 同时需要获取被unpark之后的中断状态
    ...
    /**
     * AQS提供获取锁的主要入口方法，分为以下三个步骤：
     * 1）尝试快速获取锁
     * 2）尝试获取失败之后，将线程用一个节点封装起来保存到队列当中
     * 3）加入队列之后，从队列当中获取锁，失败则阻塞，其他线程释放锁之后当前线程会从阻塞状态恢复过来
     */
    ...
    /**
     * 刚刚初始化的AQS，head和tail都是空的，假如后续执行过程中，一旦发生竞争，也就是线程入队，
     * 那么head就会初始化，所以这里head初始化置为null可以是非常有必要的
     * 但是这种实现方式，只能代表过往是否存在过竞争，而无法表示当前状态是否存在竞争
     */
